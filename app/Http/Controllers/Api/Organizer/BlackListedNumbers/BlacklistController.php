...
class BlacklistController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Prepare a message for already blocked apps
        if (count($alreadyBlockedApps) > 0) {
            // Fetch app names corresponding to the blocked app IDs from the applications table
            $blockedAppsNames = Application::whereIn('id', $alreadyBlockedApps)->pluck('name')->toArray();
            $blockedAppsList = implode(', ', $blockedAppsNames);
            return redirect()->back()->with('blacklisted_success', 'Phone number is already blacklisted for the following app(s): ' . $blockedAppsList);
        }
    ...
    public function unblock(Request $request, $phone_number)
    {
        ...
        $existingBlacklistedNumber = BlacklistedNumbers::where('phone_number', $phone_number)->first();
